...
from database import (
    ...
    movie_genres,
    movie_stars,
    MovieModel, UserGroupModel, UserGroupEnum, DirectorModel, CertificationModel
)
from database import get_db_contextmanager
from database.models.movie_models import movie_directors
...
class CSVDatabaseSeeder:
    ...
    def _preprocess_csv(self) -> pd.DataFrame:
        ...
        for col in ['stars', 'genres', 'certification', 'directors']:
        ...
        data['directors'] = data['directors'].str.replace(r'\s+', '', regex=True)
    ...
    async def _prepare_reference_data(
        ...
        """
        certifications = list(data['certification'].unique())
        genres = {
            genre.strip()
        ...
        stars = {
            actor.strip()
            for crew in data['stars'].dropna() for actor in crew.split(',')
            if actor.strip()
        }
        directors = {
            lang.strip()
            for langs in data['directors'].dropna() for lang in langs.split(',')
            if lang.strip()
        }
    ...
    def _prepare_associations(
        ...
        movie_cert_data: List[Dict[str, int]] = []
        ...
        for i, (_, row) in enumerate(tqdm(data.iterrows(), total=data.shape[0], desc="Processing associations")):
            ...
            for actor_name in row['stars'].split(','):
                actor_name = actor_name.strip()
                if actor_name:
                    actor = actor_map[actor_name]
                    movie_actors_data.append({"movie_id": movie_id, "star_id": actor.id})
            ...
            cert_name = row['certification'].strip()
            if cert_name:
                cert = language_map[cert_name]
                movie_cert_data.append({"movie_id": movie_id, "certification_id": cert.id})

        return movie_genres_data, movie_actors_data, movie_cert_data, movie_directors_data
    ...
    async def seed(self) -> None:
        ...
        try:
            ...
            cert_map, genre_map, star_map, director_map = await self._prepare_reference_data(data)
            ...
            movie_genres_data, movie_actors_data, movie_cert_data, movie_directors_data = self._prepare_associations(
                data, movie_ids, genre_map, star_map, cert_map, director_map
            )

            await self._bulk_insert(movie_genres, movie_genres_data)
            await self._bulk_insert(movie_stars, movie_actors_data)
...
async def get_csrf_cookie_and_headers(client):
    res = await client.get("/api/v1/users/csrf")
    cookie_token = res.cookies.get("fastapi-csrf-token")
    header_token = res.json().get("csrf_token")

    if not cookie_token or not header_token:
        raise AssertionError("CSRF cookie or token missing")

    headers = {"X-CSRF-Token": header_token}
    cookies = {"fastapi-csrf-token": cookie_token}
